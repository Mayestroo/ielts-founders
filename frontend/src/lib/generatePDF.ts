...
async function drawReportPage(doc: jsPDF, data: StudentReportData, logoData: LoadedImage | null) {
  ...
  doc.setFont("helvetica", "bold");
  const formattedDate = new Date(data.testDate)
  ...
  drawField(
    ...
    fieldH,
    "Family Name",
    data.student.lastName || data.student.username,
    true,
  );
  y += fieldH + 2;
  ...
  const bandScores = [lScore, rScore, wScore, sScore].filter((score) => score > 0);
  const overallBand =
    bandScores.length > 0
      ? Math.round((bandScores.reduce((sum, score) => sum + score, 0) / bandScores.length) * 2) /
        2
      : 0;
  ...
  doc.text("THIS IS NOT", pageWidth - margin - 17.5, y + 16, {
    align: "center",
  });
...
export async function generateResultPDF(data: StudentReportData) {
  ...
  const namePart =
    `${data.student.firstName || ""}_${data.student.lastName || ""}`
      .trim()
      .replace(/\s+/g, "_")
      .toUpperCase() || data.student.username.toUpperCase();
...
export async function generateBatchPDF(reports: StudentReportData[]) {
  ...
  for (let index = 0; index < reports.length; index++) {
    ...
    await drawReportPage(doc, reports[index], logoData);
  }
...
export function calculateOverallBand(results: ExamResult[]): number {
  ...
  const sum = results.reduce((accumulator, result) => accumulator + (result.bandScore || 0), 0);
