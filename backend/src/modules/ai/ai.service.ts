...
export class AiService {
  ...
  constructor(
    ...
    if (this.keys.length === 0) {
      this.logger.warn(
        'GEMINI_API_KEY not configured. AI evaluation will not work.',
      );
    } else {
  ...
  async evaluateWritingSection(
    ...
    let lastEvaluationError: Error | null = null;
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        ...
        this.logger.error(`Failed to evaluate task ${task.id}:`, errorMessage);
    ...
    if (totalWeight === 0 && tasks.length > 0) {
      ...
      throw (
        lastEvaluationError || new Error('AI evaluation failed for all tasks')
      );
    }
    ...
    if (effectiveTotalWeight > 0) {
      ...
      this.logger.log(
        `Calculated section band score: ${finalBandScore} (avg: ${average.toFixed(2)}, totalWeight: ${totalWeight}, effectiveWeight: ${effectiveTotalWeight})`,
      );
    }
  ...
  async evaluateWritingTask(
    ...
    return this.circuitBreaker.execute(async () => {
      return this.performEvaluation(taskDescription, studentResponse);
    });
  }
    ...
    for (const model of this.models) {
      ...
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          this.logger.log(
            `Attempting evaluation with model: ${model} (Key ending in ...${currentApiKey.slice(-4)})`,
          );
          ...
          const isRateLimit =
            ...
            axiosError.message?.includes('429') ||
  ...
  private buildEvaluationPrompt(
    ...
    const wordCount = studentResponse
      .trim()
      .split(/\s+/)
      .filter((w) => w).length;
...

{
  ...
  private parseEvaluationResponse(text: string): WritingEvaluation {
    ...
    try {
      ...
      const parsed = JSON.parse(jsonString) as ParsedEvaluation;
      ...
      return {
        ...
        taskAchievement: {
          ...
          feedback: String(
            parsed.taskAchievement?.feedback || 'No feedback available',
          ),
        },
        coherenceAndCohesion: {
          score: Math.max(
            0.0,
            Number(parsed.coherenceAndCohesion?.score) || 0.0,
          ),
          feedback: String(
            parsed.coherenceAndCohesion?.feedback || 'No feedback available',
          ),
        },
        lexicalResource: {
          score: Math.max(0.0, Number(parsed.lexicalResource?.score) || 0.0),
          feedback: String(
            parsed.lexicalResource?.feedback || 'No feedback available',
          ),
        },
        grammaticalRangeAndAccuracy: {
          score: Math.max(
            0.0,
            Number(parsed.grammaticalRangeAndAccuracy?.score) || 0.0,
          ),
          feedback: String(
            parsed.grammaticalRangeAndAccuracy?.feedback ||
              'No feedback available',
          ),
        },
        overallFeedback: String(
          parsed.overallFeedback || 'Evaluation completed.',
        ),
