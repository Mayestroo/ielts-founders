...
export const isWritingQueueEnabled = () => {
  if (process.env.DISABLE_WRITING_QUEUE === 'true') {
    return false;
  }

  if (process.env.ENABLE_WRITING_QUEUE === 'true') {
    return true;
  }

  if (process.env.ENABLE_WRITING_QUEUE === 'false') {
    return false;
  }
...
@Module({
  imports: writingQueueEnabled
    ...
    : [],
  exports: writingQueueEnabled ? [BullModule] : [],
})
