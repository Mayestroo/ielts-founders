...
const DASHBOARD_STATS_CACHE_PREFIX = 'cache:dashboard:stats:v1:';
const DASHBOARD_STATS_TTL_SECONDS = 25;
...
export class DashboardService {
  ...
  async getStats(user: { role: Role; centerId: string | null }) {
    const { role, centerId } = user;
    const isSuperAdmin = role === 'SUPER_ADMIN';

    const cacheKey = `${DASHBOARD_STATS_CACHE_PREFIX}role:${role}:center:${centerId || 'all'}`;
    return this.responseCache.getOrSetJson(
      cacheKey,
      ...
      async () => {
        ...
        const safeCenterFilter = isSuperAdmin
          ? {}
          : { centerId: centerId || 'non-existent-id' };
